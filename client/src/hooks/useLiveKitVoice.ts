    ...
    async function connect() {
      try {
        ...
        room.on(RoomEvent.TrackSubscribed, (track, _pub, participant) => {
          ...
          // Keep subscription bookkeeping aligned with LiveKit's actual state.
          // TrackSubscribed can fire after reconnect/resync paths where we did not
          // run the local setSubscribed(true) branch in this hook.
          ...
          const gainNode: GainNode | null = null;
        ...
        const connectOpts = { autoSubscribe: false };
        ...
        if (dist < CONNECT_RANGE && preferred.has(id) && !subscribed) {
          ...
          if (participant) {
            for (const pub of participant.trackPublications.values()) {
              ...
              if (!pub.isSubscribed) {
                // Normal case: request subscription
                (pub as RemoteTrackPublication).setSubscribed(true);
                subscribedIds.current.add(id);
              } else if (!remoteEntries.current.has(id)) {
                // LiveKit shows the track subscribed but we have no local audio entry —
                // stale subscription state from zone transition. Force a clean re-subscribe
                // so TrackSubscribed fires and rebuilds the audio element.
                console.log("[voice] stale subscription for", id, "— forcing reset");
                (pub as RemoteTrackPublication).setSubscribed(false);
                // Next loop iteration will see pub.isSubscribed=false and re-subscribe
              } else {
                // Track subscribed and entry exists — just sync bookkeeping
                subscribedIds.current.add(id);
              }
            }
          }
