...
MAP.forEach((row, r) =>
  row.forEach((id, c) => {
    TILE_GROUPS[id].push([offsetX + c * TILE_SIZE + TILE_SIZE / 2, offsetZ + r * TILE_SIZE + TILE_SIZE / 2]);
  }),
);
...
export default function FloorMap() {
  const textureUrls = useMemo(() => Object.fromEntries(UNIQUE_IDS.map((id) => [String(id), tilePath(id)])), []);
  ...
  return (
    <group>
      {UNIQUE_IDS.map((id) => (
        <TileLayer key={id} positions={TILE_GROUPS[id]} texture={textures[String(id)]} />
      ))}
    </group>
  );
}
