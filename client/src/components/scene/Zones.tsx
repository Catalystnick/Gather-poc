...
function saved(key: string, field: 'x' | 'z' | 'width' | 'height', fallback: number): number {
...
function Zone({ def }: { def: ZoneDef }) {
  ...
  const { x, z, width, height } = useControls(folder, {
    ...
    height: { value: saved(def.key, 'height', def.defaultH), min: 2,   max: 60, step: 1,   label: 'Height (tiles)' },
    Copy: button((get) => {
      const snap =
